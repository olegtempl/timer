...
// object for formate secondes,hours,minutes,days 		
		C = {
			secundes: {
				max: 60,
				min: 0,
				text: "\u0441\u0435\u043a\u0443\u043d\u0434",
				value: 0, 		// this variables varied if value == 0,timer end 
				view: !0,
				updated: !1
			},
			minutes: {
...
// this function is started after html document loaded.
...
// init type 1 or 2 or 3 
		...
		g.prototype.initType = function() {				
			...
			if (null != this.params && null != this.params.type && null != this.params.type.currentType && null != this.params.type.params) {
				...
				switch (parseInt(this.params.type.currentType)) {
					case 1:
						null != this.params.type.params.utc && (f = 0, this.params.type.params.usertime && (f = 6E4 * e.getTimezoneOffset()), f = new Date(this.params.type.params.utc + f), a = Math.round((f.getTime() - e.getTime()) / 1E3));
						break;
					case 2:
					...
					case 3:
						if (null != this.params.type.params.weekdays && null != this.params.type.params.time && null != this.params.type.params.usertime && null != this.params.type.params.hours && null != this.params.type.params.minutes) {
							...
							for (h = Math.floor(parseInt(this.params.type.params.hours) / 24); 0 <= h ? l <= h : l >= h; b = 0 <= h ? ++l : --l)
								if (d--, 0 > d && (d = 6), 1 === this.params.type.params.weekdays[d] && (a = new Date, a.setDate(a.getDate() - b), this.params.type.params.usertime || a.setTime(a.getTime() - 6E4 * e.getTimezoneOffset() + 6E4 * this.params.type.params.tz), a.setHours(parseInt(c[0])), a.setMinutes(parseInt(c[1])), a.setSeconds(0), e.getTime() >= a.getTime() && (a.setHours(a.getHours() + parseInt(this.params.type.params.hours)), a.setMinutes(a.getMinutes() + parseInt(this.params.type.params.minutes)), a.getTime() >= e.getTime()))) {
									f = a;
									break
								}
							a = null;
		...
		g.prototype.newTime = function(a) {
			...
			if (a || 0 !== this.timerElements.days.value || 0 !== this.timerElements.hours.value || 0 !== this.timerElements.minutes.value || 0 !== this.timerElements.secundes.value) {
				e = !a;
				for (f in this.timerElements) c = document.getElementById("timer-number-value-" + this.id + "-" + f), d = this.timerElements[f], l = d.value, b = !1, e && (--l, b = !0), e = !1, l < d.min && (l = d.max, b = e = !0), l >= d.max && (l = d.max - 1, b = !0), b || a ? (null != c && (c.innerHTML = l), d.value = l, d.updated = !0) : d.updated = !1;
...
/*							
					...
					CSS and DOM ( continue to end file)						
*/
...
g.prototype.initDesign = function() {
		...
		g.prototype.initPlateDesign = function() {
			...
			a = function(a) {
				return function(b, c, d) {
					...
					switch (f) {
						case "flipchart":
							e.innerHTML = "<span class='timer-flipchart-card' style='display:none;'><span class='timer-flipchart-front timer-flipchart-face'></span><span class='timer-flipchart-back timer-flipchart-face'></span><span class='timer-flipchart-top timer-flipchart-face'></span><span class='timer-flipchart-bottom timer-flipchart-face'></span><span class='timer-flipchart-bounding'>" + b + "</span></span>";
							break;
						case "slide":
			...
			f = function(a) {
				return function(a, b) {
					return "background: " + a + "; /* Old browsers */ background: -moz-linear-gradient(top, " + a + " 0%, " + b + " 100%); /* FF3.6+ */ background: -webkit-gradient(linear, left top, left bottom, color-stop(0%," + a + "), color-stop(100%," + b + ")); /* Chrome,Safari4+ */ background: -webkit-linear-gradient(top, " + a + " 0%," + b + " 100%); /* Chrome10+,Safari5.1+ */ background: -o-linear-gradient(top, " + a + " 0%," + b + " 100%); /* Opera 11.10+ */ background: -ms-linear-gradient(top, " + a + " 0%," + b + " 100%); /* IE10+ */ background: linear-gradient(to bottom, " + a + " 0%," + b + " 100%); /* W3C */ filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='" + a + "', endColorstr='" + b + "',GradientType=0);"
				}
			}(this);
			h = [];
			g = [];
			...
			switch (this.params.design.params.effect) {
				case "flipchart":
					...
					switch (c) {
						case "opacity":
							this.params.design.params.animation ? d("background-color") : (this.params.design.params["background-color"] = "rgba(110,110,100,0)", e += "#timer" + this.id + " .timer-flipchart-top:before, #timer" + this.id + " .timer-flipchart-front:before, #timer" + this.id + " .timer-flipchart-bottom:before, #timer" + this.id + " .timer-flipchart-back:before { opacity: 0!important; }");
							break;
						case "solid":
							this.params.design.params.animation && d("background-color");
							break;
						case "gradient":
							d = this.params.design.params["background-color"], null != d && "object" === typeof d && 1 < d.length && (this.params.design.params.animation && (d[0] = k(d[0])), this.params.design.params.animation && (d[1] = k(d[1])), l = "#timer" + this.id + " .timer-flipchart-top, #timer" + this.id + " .timer-flipchart-front {" + f(d[0], d[1]) + "}" + ("#timer" + this.id + " .timer-flipchart-bottom, #timer" + this.id + " .timer-flipchart-back {") + f(d[1], d[0]) + "}")
					}
					e += "#timer" + this.id + " .timer-flipchart-face{" + b("background-color", "background-color") + this.getCssFonts("number") + "}" + ("#timer" + this.id + " .timer-separator{") + b("padding-top", "padding", "px!important") + b("padding-bottom", "padding", "px!important") + "}" + ("#timer" + this.id + " .timer-flipchart-bounding{") + b("padding-left", "padding", "px") + b("padding-right", "padding", "px") + b("padding-top", "padding", "px") + b("padding-bottom", "padding", "px") + "}" + ("#timer" + this.id + " .timer-flipchart-top, #timer" + this.id + " .timer-flipchart-front{") + b("padding-left", "padding", "px") + b("padding-right", "padding", "px") + b("padding-top", "padding", "px") + b("padding-bottom", "padding", "px", 0) + "}" + ("#timer" + this.id + " .timer-flipchart-bottom, #timer" + this.id + " .timer-flipchart-back{") + b("padding-left", "padding", "px") + b("padding-right", "padding", "px") + b("padding-top", "padding", "px", 0) + b("padding-bottom", "padding", "px") + "}" + ("#timer" + this.id + " .timer-flipchart-top, #timer" + this.id + " .timer-flipchart-front {") + b("border-top-left-radius", "round", "px") + b("border-top-right-radius", "round", "px") + b("border-bottom-left-radius", "", "px", 0) + b("border-bottom-right-radius", "", "px", 0) + "}" + ("#timer" + this.id + " .timer-flipchart-bottom, #timer" + this.id + " .timer-flipchart-back {") + b("border-top-left-radius", "", "px", 0) + b("border-top-right-radius", "", "px", 0) + b("border-bottom-left-radius", "round", "px") + b("border-bottom-right-radius", "round", "px") + "}" + ("#timer" + this.id + " .timer-flipchart-bottom:after {") + b("border-top-left-radius", "", "px", 0) + b("border-top-right-radius", "", "px", 0) + b("border-bottom-left-radius", "round", "px") + b("border-bottom-right-radius", "round", "px") + "}" + ("#timer" + this.id + " .timer-flipchart-front:after, #timer" + this.id + " .timer-flipchart-top:after{") + b("border-top-left-radius", "round", "px") + b("border-top-right-radius", "round", "px") + b("border-bottom-left-radius", "", "px", 0) + b("border-bottom-right-radius", "", "px", 0) + "}" + ("#timer" + this.id + " .timer-flipchart-card {") + b("border-radius", "round", "px") + "}" + l;
		...
		g.prototype.getCssFonts = function(a) {
			...
			null == a && (a = "");
			b = "";
			if (null != this.params.design && null != this.params.design.params && (c = this.params.design.params[a + "-font-family"], d = this.params.design.params[a + "-font-size"], a = this.params.design.params[a + "-font-color"], null != d && 0 <= parseInt(d) && (b += "font-size:" + parseInt(d) + "px;"), null != a && (b += "color:" + a + ";"), null != c)) return c = null != c.family ? c.family : null, b + ("font-family:'" + c + "';")
		};
		g.prototype.setFonts = function(a, b) {
			var c, d, e, f, l, h, k;
			null == b && (b = "");
			if (null != this.params.design && null != this.params.design.params && (d = this.params.design.params[b + "-font-family"], e = this.params.design.params[b + "-font-size"], c = this.params.design.params[b + "-font-color"], null != d && (null != d.link && this.addCssLink(d.link), d = null != d.family ? d.family : null), null != a && 0 < a.length)) {
				k = [];
				l = 0;
				...
				for (m = k.length; e < m; e++) {
					...
					switch (this.params.design.params.effect) {
						case "flipchart":
							...
							null != d && null != d.childNodes && 1 === d.childNodes.length && (a = d.childNodes[0].childNodes) && null != a && 5 === a.length && a[0].innerHTML !== h && (c = parseInt(a[1].innerHTML), null != c && 0 <= c && 9 >= c || (c = ""), this.params.design.params.animation || (c = h), c === h && this.params.design.params.animation || (a[0].innerHTML = c, a[1].innerHTML = h, a[2].innerHTML = h, a[3].innerHTML = c, g(b, f)));
							break;
						case "slide":
							d = document.getElementById("timer-number-slide" + b + "-" + this.id + "-" + f), null != d && null != d.childNodes && 3 === d.childNodes.length && (a = d.childNodes) && a[1].innerHTML !== h && (c = parseInt(a[1].innerHTML), null != c && 0 <= c && 9 >= c || (c = ""), this.params.design.params.animation || (c = h), c === h && this.params.design.params.animation || (a[0].innerHTML = c, a[1].innerHTML = h, g(b, f)))
					}
					u.push(b++)
				}
				return u
		...
		g.prototype.isEqualsNodes = function(a, b) {
			...
			if (null != a && null != b)
				...
				d = 0;
				for (e = f.length; d < e; d++)
					if (c = f[d], (g = !c, 0 <= B.call(b.attributes, g)) || c.value !== b.getAttribute(c.nodeName)) return !1;
				f = b.attributes;
				d = 0;
				for (e = f.length; d < e; d++)
